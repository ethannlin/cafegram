...
def discover_create():
    ...
    if 'update' in request.form and request.form.get('update') == 'on':
        user = get_user(session)
        if user is None:
        ...
        user = get_user(session)
        if user is None:
            render_template('discover.html', title='discover', token=session['token'], track_ids=[], error='Error: Could not retrieve user.')
