...
function tokenNeedsRefresh() {
    ...
    return (
        !expirationTime || currentTime >= expirationTime - tokenRefreshThreshold
    );
}
...
window.onSpotifyWebPlaybackSDKReady = () => {
    ...
    $("#postion-slider").on("input", function () {
        ...
        window.spotifyPlayer.seek(position_ms);
    });
    ...
    window.spotifyPlayer.connect().then((success) => {
        if (success) {
            ...
            $("#volume-slider").on("input", function () {
                ...
                window.spotifyPlayer.setVolume(volumeValue).then(() => {
...
$("#repeat").on("click", function () {
    ...
    if (repeatFontWeight === "400") {
        ...
        repeatFontWeight === "700" &&
        repeatTextDecoration.includes("none")
    ) {
        state = "track";
    } else if (
        repeatFontWeight === "700" &&
        repeatTextDecoration.includes("underline")
    ) {
        state = "off";
    }
...
$(document).ready(function () {
    $("#search-input").autocomplete({
        ...
        select: function (event, ui) {
            ...
            endpointURL = `/api/play/${$("#search-type").val()}/${
                ui.item.value
            }`;
