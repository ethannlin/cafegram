...
$("#createBtn").click(function () {
    ...
    if (counter == 0) {
        ...
        sliders.forEach((slider) => {
            ...
            if (
                sliderName == "slider_popularity" ||
                sliderName == "slider_limit"
            ) {
                slider.value = 50;
            } else {
                slider.value = 0.5;
        ...
        slider_values.forEach((slider) => {
            if (
                slider.getAttribute("name") === "popularity_val" &&
                slider.getAttribute("name") === "limit_val"
            ) {
                slider.innerHTML = 50;
            } else {
                slider.innerHTML = 0;
    ...
    if ($("#selectedItems").find('[data-id="' + value + '"]').length === 0) {
        ...
        if ($("#selectedItems").children().length < 5) {
            ...
            $("#selectedItems").append(
                '<a class="list-group-item"><span data-id="' +
                    value +
                    '">' +
                    label +
                    '</span><span><span class="btn btn-xs btn-default" style="float: right;" onclick="removeSelected(this);">&times;</span></span></a>'
            );
        } else {
